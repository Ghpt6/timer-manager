...
object TimerAlarms {
    private const val CHANNEL_ID = "kitchen_timer_finished"
    ...
    fun reminderWarning(context: Context): String? = when {
        !notificationsEnabled(context) -> "通知未开启，锁屏后无法提醒。"
        !canScheduleExact(context) -> "精确提醒未开启，到时通知可能延迟。"
        else -> null
    }

    private fun launchIntent(context: Context, id: Int): PendingIntent = PendingIntent.getActivity(
        context, id,
    ...
    private fun deliver(context: Context, id: Int, timer: JSONObject) {
        val deadline = timer.optLong("deadline", 0)
        ...
        if (!notificationsEnabled(context)) {
            if (MainActivity.isVisible) {
                ...
                Handler(Looper.getMainLooper()).postDelayed({ tone.release() }, 1500)
            }
            return
        }
